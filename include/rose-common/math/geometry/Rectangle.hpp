...
namespace ROSECOMMON_MATH_NAMESPACE
{
	...
	public:
		 ...
		 */
		constexpr RectangleT(const Vector2T<T>& aPoint, const Vector2T<T>& aSize) noexcept;
		 ...
		 */
		static constexpr RectangleT FromPoints(const Vector2T<T>& aPointA, const Vector2T<T>& aPointB) noexcept;
		 ...
		 */
		static constexpr RectangleT FromPointAndOffset(const Vector2T<T>& aPoint, const Vector2T<T>& aVector) noexcept;
		 ...
		 */
		constexpr Vector2T<T> Size() const;
		 ...
		 */
		constexpr Vector2T<T> TopLeft() const;

		/**
		 * @brief Get the position of the rectangle's top-right corner.
		 */
		constexpr Vector2T<T> TopRight() const;
		 ...
		 */
		constexpr bool Contains(const Vector2T<T>& aPoint) const;
		 ...
		 */
		void Inflate(const Vector2T<T>& aSize);
		 ...
		 */
		void Union(const Vector2T<T>& aPoint);
	...
	template <typename T>
	constexpr RectangleT<T>::RectangleT(const Vector2T<T>& aPoint, const Vector2T<T>& aSize) noexcept
		: X(aPoint.X)
		, Y(aPoint.Y)
		, Width(aSize.Width)
		, Height(aSize.Height)
	...
	template <typename T>
	constexpr RectangleT<T> RectangleT<T>::FromPoints(const Vector2T<T>& aPointA, const Vector2T<T>& aPointB) noexcept
	{
		...
		return RectangleT<T>({ x, y }, { width, height });
	}

	template <typename T>
	constexpr RectangleT<T> RectangleT<T>::FromPointAndOffset(const Vector2T<T>& aPoint, const Vector2T<T>& aVector) noexcept
	{
	...
	template<typename T>
	constexpr Vector2T<T> RectangleT<T>::BottomLeft() const
	{
		return Vector2T<T>(Left(), Bottom());
	}

	template<typename T>
	constexpr Vector2T<T> RectangleT<T>::BottomRight() const
	{
		return Vector2T<T>(Right(), Bottom());
	}

	template<typename T>
	constexpr Vector2T<T> RectangleT<T>::Center() const
	{
		return Vector2T<T>(X + (Width / 2), Y + (Height / 2));
	}
	...
	template<typename T>
	constexpr Vector2T<T> RectangleT<T>::Size() const
	{
	...
	template<typename T>
	constexpr Vector2T<T> RectangleT<T>::TopLeft() const
	{
		return Vector2T<T>(Left(), Top());
	}

	template<typename T>
	constexpr Vector2T<T> RectangleT<T>::TopRight() const
	{
		return Vector2T<T>(Right(), Top());
	}

	template<typename T>
	constexpr bool RectangleT<T>::Contains(const Vector2T<T>& aPoint) const
	{
		return
	...
	template <typename T>
	void RectangleT<T>::Inflate(const Vector2T<T>& aSize)
	{
	...
	template <typename T>
	void RectangleT<T>::Union(const Vector2T<T>& aPoint)
	{
